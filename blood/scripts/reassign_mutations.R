#script provided by Mike Spencer

include_ancestral_tip=TRUE
#========================================#
...
# Set file paths ####
#========================================#
tree_file_path="/workspace/projects/lesion_segregation/blood/data/tree_PX001_2_01_standard_rho01.tree"
filtered_muts_file="/workspace/projects/lesion_segregation/blood/data/annotated_mut_set_PX001_2_01_standard_rho01"
...
if(re_run|!all(c("node","pval")%in%colnames(details))|sum(tree$edge.length)!=nrow(details)|any(!details$node%in%tree$edge[,2])) {
  ...
  save(filtered_muts,file=paste(filtered_muts_file, ".reassigned", sep=""))
  write.tree(tree,file = paste(tree_file_path, ".reassigned", sep=""))
}
