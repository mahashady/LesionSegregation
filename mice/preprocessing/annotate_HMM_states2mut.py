...
def merge_HMMas(sample_name):
    df_HMM_input = pd.read_csv("../../MRCA/results/HMM/input_HMM/" + sample_name + ".hmm", sep=" ", header=None)
    df_HMM_input.columns = ['Chr', 'Pos', 'Change', 'Context', 'Nref', 'Nalt', 'Sample','Multi_class','As_class']
    df_HMM_multi = pd.read_csv("../../MRCA/results/HMM/output_HMM/" + sample_name + ".hmm.PloidyHMM", header=None)
...
def main(args):
    ...
    for sample_name in samples:
        ...
        df_input = pd.read_csv("../../data/mutations_vs_genes/" + sample_name + ".with_gene_annot.nodMat", sep=",")
        print(df_input.head())
