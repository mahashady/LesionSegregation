...
u_range = np.arange(0.05, 0.6, 0.1)
...
for s in s_range:
    ...
    for e in efr_range:
        for r in r_range:
            for u in u_range:
                run = True
                simulation_path = f'{lesions_path}/epistasis_add_driver_on_nontx_strands_efr_{round(e, 2)}_eft_0.0_2strands_True_repair_{round(r, 1)}_{round(u, 2)}_lesion_mut/'
                params_row = [round(s, 1), round(e, 2), round(r, 1), round(u, 2), round(r-u, 2)]
                if not os.path.isdir(simulation_path):
                    simulation_path = f'{lesions_path}/epistasis_add_driver_on_tx_strands_efr_{round(e, 2)}_eft_0.0_2strands_True_repair_{round(r, 1)}_{round(u, 2)}_lesion_mut/'
                    if not os.path.isdir(simulation_path):
                        run = False
    ...
    df.to_csv(f'{outdir}/s_{round(s, 2)}.csv', index=False)
